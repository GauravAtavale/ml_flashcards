...
if "flashcards" not in st.session_state: #or len(st.session_state.flashcards)==0
    st.session_state.index = 0
    st.session_state.orig_option = 'Stat'

st.session_state.option = st.selectbox('What do you want to revise?',('Stat','ML', 'DS'))
# Adds a horizontal line
st.divider() 

if st.session_state.option != st.session_state.orig_option:
    st.session_state.orig_option = st.session_state.option
    st.session_state.index = 0
    # st.write('You selected:', st.session_state.option,st.session_state.orig_option,st.session_state.index)
if st.session_state.option == 'ML':
    pick_file = 'ml_flashcard_data_v1.txt'
    st.session_state.flashcards =[]

elif st.session_state.option == 'Stat':
    pick_file = 'QA_stats_v1.txt'
    st.session_state.flashcards =[]

st.session_state.flashcards = load_flashcards(pick_file) # QA_stats.txt
...
if st.session_state.flashcards:
    ...
    # Adding a Hint button -- Hint is commented out. Can be added if needed
    # if st.button("Hint"):
    #     st.write("Hint is given")
